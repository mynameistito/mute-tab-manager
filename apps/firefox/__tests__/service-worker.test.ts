...
describe("initDarkModeDetection", () => {
...
describe("runtime.onInstalled listener", () => {
  test("creates both context menus and initializes dark mode detection", async () => {
    const mq = createMockMQ(false);
    (globalThis as Record<string, unknown>).matchMedia = () => mq;
  ...
  test("sets isDarkMode true when system is dark", async () => {
    const mq = createMockMQ(true);
    (globalThis as Record<string, unknown>).matchMedia = () => mq;
    await mockEvents.runtime.onInstalled.fire({
      reason: "install" as chrome.runtime.OnInstalledReason,
